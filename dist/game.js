class Game {
    ...
    play(move) {
        ...
        this.log.push(move);
        this.meta.plyCount++;
        const x = this.node.lastX() === 13, o = this.node.lastO() === 3;
        if (x && !o) {
...
[Result "${this.meta.result}"]\n\n`;
    ...
    renderBoard(move) {
        ...
        for (let i = 0; i < 9; i++) {
            for (let j = 0; j < 9; j++) {
                const piece = this.node.board[i][j], style = U.cell(j, i).style;
